...
<head>
    ...
    <title>Verifikasi Surat Keterangan Usaha</title>
    <style>
        body {
...
<body>
    <div class="container">
        <div class="header">
            <h1>VERIFIKASI SURAT KETERANGAN USAHA</h1>
            <p>Sistem Verifikasi Digital Desa Contoh</p>
        </div>
        ...
        @if($valid && isset($sku))
            ...
            <table class="data-table">
                ...
                <tr>
                    ...
                    <td>{{ $sku->status_verifikasi }}</td>
                </tr>
                <tr>
                    <td><strong>Tanggal Dibuat</strong></td>
                    <td>{{ $sku->created_at->translatedFormat('d F Y') }}</td>
                </tr>
            </table>

            <div style="margin-top: 30px; padding: 15px; background-color: #e9ecef; border-radius: 5px;">
                <h3>Informasi Surat</h3>
                <p><strong>Nomor Surat:</strong>  @if($sku->nomor_surat)
                    {{ $sku->nomor_surat }}
                @else
                    {{ $sku->id }}/SKU/{{ date('Y') }}
                @endif
